...
extension AppDelegate:MessagingDelegate {
    func messaging(_ messaging: Messaging, didReceiveRegistrationToken fcmToken: String?) {
        ...
        Constants.staticKeys.USER_DEFAULTS.set(token, forKey: Constants.staticKeys.FCMtoken)
...
extension AppDelegate: UNUserNotificationCenterDelegate {
    ...
    func userNotificationCenter(_ center: UNUserNotificationCenter, didReceive response: UNNotificationResponse, withCompletionHandler completionHandler: @escaping () -> Void) {
        ...
        if(application.applicationState == .active){
            if (content.title.contains("Join Session")){
                ...
                guard let invite_code = content.userInfo["invite_code"] as? String else { return }
                guard let startedAt = content.userInfo["starts_at"] as? String else { return }
                guard let hostDisplayName = content.userInfo["host_display_name"] as? String else { return }
                guard let hostUsername = content.userInfo["host_username"] as? String else { return }
                guard let hostProfileImage = content.userInfo["host_profile_image"] as? String else { return }
                guard let roomStatus = content.userInfo["room_status"] as? String else { return }
                guard let imgURL = Constants.APIUrls.Resources+hostProfileImage as? String else { return }
                
                let sessionInfo = Room(roomName: room_name, roomID: room_id, inviteCode: invite_code, startsAt: startedAt, hostDisplayName: hostDisplayName, hostUsername: hostUsername, hostProfileImage: imgURL, roomStatus: roomStatus)
                
                coordinateToRoomPlayListVC(info:sessionInfo)
            }
            print("user tapped the notification bar when the app is in foreground")
    ...
    private func coordinateToRoomPlayListVC(info:Room){
        guard let window = UIApplication.shared.keyWindow else { return }
        let storyboard = UIStoryboard(name: "Room", bundle: nil)
        let main = UIStoryboard(name: "Main", bundle: nil)
        guard let vc = storyboard.instantiateViewController(identifier: "RoomPlayListVC") as? RoomPlayListVC else { return }
        guard let Root = main.instantiateViewController(withIdentifier: "JamSessionVC") as? JamSessionVC else { return }
        let navController = UINavigationController()
        navController.viewControllers = [Root,vc]
        vc.fromNotification = true
        vc.AsGuest = true
        vc.sessionDetails = info
        navController.modalPresentationStyle = .fullScreen
        navController.navigationBar.tintColor = UIColor(named: "JAM")
